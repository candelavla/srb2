...
void P_DoSuperTransformation(player_t *player, boolean giverings)
{
	...
	S_StartSoundFromMobj(player->mo, sfx_supert);
	...
	if ((cmd->buttons & BT_JUMP || player->rsprung == -1) && !player->exiting && !P_PlayerInPain(player))
	{
		if (player->powers[pw_springlock] && (!(player->pflags & (PF_JUMPED|PF_NOJUMPDAMAGE|PF_THOKKED|PF_SHIELDABILITY))) && (!(player->charflags & SF_TRICKFORBIDDEN)))
		{
			if (abs(player->rsprung) == 1 && (!(player->pflags & PF_JUMPDOWN)))
			{
				...
				S_StartSoundFromMobj(player->mo, sfx_s249);
				player->pflags |= PF_FULLSTASIS;
				player->flycmd = 3;
			}
			player->pflags |= PF_JUMPDOWN;
			...
			switch (player->charability)
			{
				...
				case CA_JUMPTHOK: //jumpthok
					...
					if (!player->secondjump)
					{
						...
						P_SpawnThokMobj(player);
						S_StartSoundFromMobj(player->mo, sfx_thok);
...
void P_MovePlayer(player_t *player)
{
	...
	|| (player->charability2 == CA2_MELEE && P_IsPlayerInState(player, S_PLAY_MELEE_LANDING) && P_IsObjectOnGround(player->mo)))
		player->pflags |= PF_FULLSTASIS;
...
consvar_t cv_cam2_delay = CVAR_INIT ("cam2_delay", "True", CV_SAVE|CV_ALLOWLUA, CV_TrueFalse, NULL);
