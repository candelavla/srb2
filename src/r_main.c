...
consvar_t cv_drawdist_precip = CVAR_INIT ("drawdist_precip", "2048", CV_SAVE, drawdist_precip_cons_t, NULL);
consvar_t cv_fov = CVAR_INIT ("fov", "100", CV_SAVE|CV_FLOAT|CV_CALL, fov_cons_t, Fov_OnChange);
consvar_t cv_fovadjust = CVAR_INIT ("fovadjust", "Off", CV_SAVE|CV_CALL, CV_OnOff, Fov_OnChange);
...
//
INT32 R_OldPointOnSide(fixed_t x, fixed_t y, bspnode_t *restrict node)
{
	if (!node->dx)
